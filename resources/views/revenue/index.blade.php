...
@endsection 
  ...
  <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg ">
    <nav class="navbar navbar-main navbar-expand-lg px-0 mx-4 shadow-none border-radius-xl" id="navbarBlur" navbar-scroll="true">
        ...
        <div class="w-50 d-flex justify-content-evenly align-items-center mt-2">
          <a href="{{route('revenue.create')}}"> 
            <button class="btn btn-primary mt-2" >Create revenue</button>  
          </a>
          <a href="{{url('/revenue/search')}}">
            <button class="btn btn-primary mt-2 ms-3">deep research</button>
          </a>
          <button class="btn  mt-2 ms-3 fw-bold"  style="background-color: rgb(12, 12, 104);color:aliceblue;font-size:12px;">TOTAL PAIMENT : {{number_format($sum)}} DH</button> 
        </div> 
    </nav> 
            ...
            <div class="card my-4">
              <div class="card-header p-0 position-relative mt-n5 mx-3 z-index-2">
                <div class="bg-gradient-primary shadow-primary border-radius-lg pt-4 pb-3">
                  <h6 class="text-white text-capitalize ps-3">revenue table</h6>
                </div>
              </div>
                  ...
                  <table class="table align-items-center mb-0 mt-n-5">
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td class="align-middle text-center">
                          <form method="POST" action="{{route('revenue.edit',$item->id)}}"> 
                            {{csrf_field()}} 
                            @method('GET') 
                        ...
                        <td class="align-middle text-center">
                          <form action="{{route('revenue.destroy',$item->id)}}" method="POST"> 
                            @csrf
                            @method('DELETE')
                            <input type="hidden" name="id" value="{{$item->id}}"> 
